...
export function BehaviorsScreen({ projectId, initialBehaviorId, t, onError }: Props) {
  ...
  const [selectedRuntimeProfileVersion, setSelectedRuntimeProfileVersion] = useState("");
  ...
  const refresh = async () => {
    const [nextBehaviors, nextRuntimes, nextCaptures, nextProfiles] = await Promise.all([
      listBehaviors(projectId),
      listRuntimes(projectId),
      listCaptures(projectId),
      listRuntimeProfiles(projectId),
    ]);
    ...
    setSelectedRuntimeProfileVersion((current) => current || nextProfiles.find((item) => item.current_version.approved_at)?.current_version_id || "");
  };
  ...
  const reviewCapture = behaviorCaptures.find((capture) => ["REVIEW_REQUIRED", "VALIDATED"].includes(capture.status)) ?? null;
  ...
  const cancelCurrentCapture = () => void run(async () => {
    if (!activeCapture) return;
    await cancelCapture(projectId, activeCapture.id);
    setBundle(null);
    await refresh();
  });
  ...
  const validateBaseline = () => void run(async () => {
    if (!reviewCapture || !selectedRuntimeProfileVersion) return;
    await submitCaptureReview(projectId, reviewCapture.id, [{
      type: "TEXT_CONTAINS",
      value: selected?.current_version.expected_outcome ?? "",
    }], reviewNotes);
    await validateCapture(projectId, reviewCapture.id, selectedRuntimeProfileVersion);
    await refresh();
  });
  ...
  const summary = {
    ...
    protected: behaviors.filter((item) => item.lifecycle_state === "KNOWN_GOOD").length,
    ...
    <div className="behavior-workspace">
      ...
      <div className="behavior-main">
        <section className="panel behavior-editor">
          ...
          <div className="button-row"><button className="primary" disabled={busy || !draft.title.trim() || selected?.lifecycle_state === "DISABLED"} onClick={saveDraft}>{busy ? t("common.working") : selected ? t("behavior.saveVersion") : t("behavior.createDraft")}</button></div>
        </section>
        {selected && <>
          <section className="panel">
            <div className="section-head"><h2>{t("behavior.current")}</h2><span className={`readiness ${selected.lifecycle_state === "KNOWN_GOOD" ? "good" : "neutral"}`}>{t(`behavior.state.${selected.lifecycle_state}` as TranslationKey)}</span></div>
            ...
            <div className="button-row"><button className="secondary danger" disabled={busy || selected.lifecycle_state === "DISABLED"} onClick={() => void run(async () => { await archiveBehavior(projectId, selected.id); await refresh(); })}>{t("behavior.archive")}</button></div>
          ...
          <section className="panel runtime-panel">
            ...
            {runtimeProfiles.length > 0 && <label className="field"><span>{t("capture.approvedRuntimeProfile")}</span><select value={selectedRuntimeProfileVersion} onChange={(event) => setSelectedRuntimeProfileVersion(event.target.value)}>{runtimeProfiles.filter((profile) => profile.current_version.approved_at).map((profile) => <option value={profile.current_version_id} key={profile.id}>{profile.display_name} · {profile.current_version.runtime_type}</option>)}</select></label>}
          </section>
          <section className="panel capture-panel">
            ...
            {!activeCapture && !reviewCapture && <button className="primary" disabled={busy || !selectedRuntime || selected.lifecycle_state === "DISABLED"} onClick={beginCapture}>{t("capture.start")}</button>}
            {activeCapture && <div className="capture-live" role="status" aria-live="polite"><img className="mascot-art mascot-helper-art" src={mascotAssets["yak-working-laptop"].src} alt={t(mascotAssets["yak-working-laptop"].altKey)} /><strong>{t("capture.recording")}</strong><span>{activeCapture.entry_url}</span><div className="metric-grid"><div><strong>{activeCapture.steps.length}</strong><span>{t("capture.steps")}</span></div><div><strong>{activeCapture.observations.length}</strong><span>{t("capture.observations")}</span></div><div><strong>{t("common.localOnly")}</strong><span>{t("capture.privacy")}</span></div></div><div className="button-row"><button className="secondary" disabled={busy} onClick={() => void run(async () => { activeCapture.paused ? await resumeCapture(projectId, activeCapture.id) : await pauseCapture(projectId, activeCapture.id); await refresh(); })}>{activeCapture.paused ? t("capture.resume") : t("capture.pause")}</button><button className="secondary danger" disabled={busy} onClick={cancelCurrentCapture}>{t("common.cancel")}</button><button className="primary" disabled={busy} onClick={finishCapture}>{t("capture.stopReview")}</button></div></div>}
