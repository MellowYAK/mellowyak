...
def test_valid_token_and_first_setup_values(tmp_path: Path) -> None:
    ...
    assert body["database_schema_version"] == "0001_local_core"
    ...
    assert installation["database_schema_version"] == "0001_local_core"
...
def test_startup_performs_no_outbound_network(monkeypatch, tmp_path: Path) -> None:
    ...
    try:
        ...
        assert app.state.runtime.schema_version == "0001_local_core"
        assert attempted == []
    finally:
